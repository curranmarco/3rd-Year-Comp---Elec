...
int hash2(char* s){ 
    int hash = 0;
    //we will use this small prime number to multiply it by the hash number to reduce the number of collisions
    int primen = 31;
    while(*s){
    hash = (hash * primen + (*s - 'A' + 52)) % ARRAY_SIZE;
    s++; 
    }
    return hash;
}
